...
# =============================================================================
# Train (optimized parameters)
#
# Key changes from base:
#   - 30 epochs (vs 1) for convergence
#   - Adam optimizer with learning rate schedule (vs flat RMSProp)
#   - EarlyStopping to prevent overfitting
#   - ModelCheckpoint to save best weights by val_loss
#   - ReduceLROnPlateau to lower LR when progress stalls
#   - Custom callback: saves attention heatmap every epoch
# =============================================================================
...
# Pick a fixed sentence for attention tracking across epochs.
# Using a fixed sentence lets us see how attention patterns evolve during training.
attention_tracking_sentence = train_spa_texts[0]
print(f"\nTracking attention evolution for: '{attention_tracking_sentence}'")

# Create output folder for per-epoch heatmaps
import os
heatmap_dir = "attention_per_epoch"
os.makedirs(heatmap_dir, exist_ok=True)


class AttentionHeatmapCallback(keras.callbacks.Callback):
    """Saves an attention heatmap at the end of each epoch.

    This lets us visualize how attention patterns evolve during training:
    - Early epochs: near-uniform attention (model hasn't learned yet)
    - Mid epochs: patterns start emerging (some tokens attend to related ones)
    - Late epochs: sharp, distinct patterns (model has learned relationships)
    """

    def __init__(self, sentence, save_dir, head=0):
        super().__init__()
    ...
    def on_epoch_end(self, epoch, logs=None):
        # Tokenize
        tokenized = spa_vectorization([self.sentence])
        ...
        # Get attention weights (eager mode)
        pos_embed_layer = self.model.get_layer("positional_embedding")
        encoder_layer = self.model.get_layer("transformer_encoder")

        embedded = pos_embed_layer(tokenized)
        _, attention_weights = encoder_layer(embedded, return_attention_scores=True)

        attn_matrix = ops.convert_to_numpy(
            attention_weights[0, self.head, :seq_len, :seq_len]
        )
        ...
        ax.set_title(f"Self-attention head {self.head} — Epoch {epoch + 1}")
        plt.colorbar(im, ax=ax, label="Attention weight")
        plt.tight_layout()

        filepath = os.path.join(self.save_dir, f"epoch_{epoch + 1:02d}.png")
        fig.savefig(filepath, dpi=150)
        plt.close(fig)  # Close to avoid memory buildup
        print(f"  Saved heatmap: {filepath}")


# Adam with a tuned learning rate — 1e-4 is a good starting point for transformers.
...
callbacks = [
    ...
    # Halve the learning rate if val_loss plateaus for 2 epochs
    ...
    # Save attention heatmap every epoch
    AttentionHeatmapCallback(
        sentence=attention_tracking_sentence,
...
# =============================================================================
# Plot training curves (loss, accuracy, and learning rate)
# =============================================================================

num_epochs_ran = len(history.history["loss"])
epoch_range = range(1, num_epochs_ran + 1)

fig, axes = plt.subplots(1, 3, figsize=(20, 5))

# Loss
axes[0].plot(epoch_range, history.history["loss"], "b-o", markersize=4, label="Train Loss")
axes[0].plot(epoch_range, history.history["val_loss"], "r-o", markersize=4, label="Val Loss")
...
# Accuracy
axes[1].plot(epoch_range, history.history["accuracy"], "b-o", markersize=4, label="Train Accuracy")
axes[1].plot(epoch_range, history.history["val_accuracy"], "r-o", markersize=4, label="Val Accuracy")
...
if lr_key:
    axes[2].plot(epoch_range, history.history[lr_key], "g-o", markersize=4, label="Learning Rate")
    ...
    # If LR not tracked, show train vs val gap (overfitting indicator)
    gap = [t - v for t, v in zip(history.history["accuracy"], history.history["val_accuracy"])]
    axes[2].plot(epoch_range, gap, "m-o", markersize=4, label="Train - Val Accuracy")
...
#
# Plot attention heatmaps for all heads to show how different heads
# learn different attention patterns (e.g. positional, semantic, syntactic).
# =============================================================================

def get_attention_weights(sentence):
    """Get encoder self-attention weights for a sentence (eager mode)."""
    ...
    # Run through encoder layers directly (eager mode)
    ...
    return ops.convert_to_numpy(attention_weights), token_labels, seq_len


def plot_attention_single(sentence, head=0):
    """Plot a single head's attention heatmap (matching assignment example)."""
    attention_weights, token_labels, seq_len = get_attention_weights(sentence)
    attn_matrix = attention_weights[0, head, :seq_len, :seq_len]
    ...
    plt.show()
    print(f"Saved attention_head_{head}.png")


def plot_attention_all_heads(sentence):
    """Plot all heads in a grid to compare what each head learns."""
    attention_weights, token_labels, seq_len = get_attention_weights(sentence)

    fig, axes = plt.subplots(2, num_heads // 2, figsize=(5 * (num_heads // 2), 10))
    axes = axes.flatten()

    for head in range(num_heads):
        attn_matrix = attention_weights[0, head, :seq_len, :seq_len]
        im = axes[head].imshow(attn_matrix, cmap="viridis")
        axes[head].set_xticks(range(seq_len))
        axes[head].set_yticks(range(seq_len))
        axes[head].set_xticklabels(token_labels, rotation=45, ha="right", fontsize=7)
        axes[head].set_yticklabels(token_labels, fontsize=7)
        axes[head].set_title(f"Head {head}", fontsize=10)

    plt.suptitle(f"Self-attention across all {num_heads} heads", fontsize=14)
    plt.tight_layout()
    plt.savefig("attention_all_heads.png", dpi=150)
    plt.show()
    print("Saved attention_all_heads.png")


# Visualize attention for a test sentence
test_sentence = random.choice(test_eng_texts)
print(f"\nVisualizing attention for: '{test_sentence}'")

# Single head (matches assignment example)
plot_attention_single(test_sentence, head=0)

# All heads comparison (bonus for report)
plot_attention_all_heads(test_sentence)